...
export const TestsTab: React.FC<TestsTabProps> = ({
    ...
    return (
      <div style={styles.container}>
        <div style={styles.emptyCard}>
          <h3>No Test Run Available</h3>
          <p>Generate tests to view coverage metrics, uncovered lines, and execution status.</p>
          {onGenerateUncovered && (
            <button
            ...
            >
              {loading ? 'Generating...' : 'Generate Tests for Uncovered Code'}
            </button>
          )}
        </div>
      </div>
  ...
  return (
    <div style={styles.container} data-testid="tests-tab">
      ...
      <div style={styles.summaryCard}>
        ...
        <div style={styles.metricsGrid}>
          <div style={styles.metricItem}>
            <span style={styles.metricLabel}>Tests Generated</span>
            <span style={styles.metricValue}>{testRunData.testsGenerated}</span>
          </div>
          <div style={styles.metricItem}>
            <span style={styles.metricLabel}>Passed</span>
            <span style={{ ...styles.metricValue, color: '#10b981' }}>
              {testRunData.testsPassed}
            </span>
          </div>
          <div style={styles.metricItem}>
            ...
            <span
              style={{
                ...styles.metricValue,
                color: testRunData.testsFailed > 0 ? '#ef4444' : '#9ca3af',
              }}
            >
              {testRunData.testsFailed}
            </span>
          </div>
          <div style={styles.metricItem}>
            ...
            <span
              style={{
                ...styles.metricValue,
                color: linePercent >= testRunData.target ? '#10b981' : '#f59e0b',
              }}
              data-testid="line-coverage-value"
            >
              {testRunData.lineCoverage}%
            </span>
          </div>
          <div style={styles.metricItem}>
            <span style={styles.metricLabel}>Branch Coverage</span>
            <span style={styles.metricValue} data-testid="branch-coverage-value">
              {testRunData.branchCoverage}%
            </span>
          </div>
        </div>
        ...
        <div style={styles.progressSection}>
          <div style={styles.progressRow}>
            <span style={styles.progressLabel}>
              Line Coverage ({linePercent}% / {testRunData.target}% target)
            </span>
            <div style={styles.progressTrack}>
              <div
                style={{
                  ...styles.progressBar,
                  width: `${linePercent}%`,
                  backgroundColor: linePercent >= testRunData.target ? '#10b981' : '#f59e0b',
                }}
              />
            </div>
          </div>
      ...
      {/* Main Content Grid: Uncovered Lines & Code */}
      <div style={styles.contentGrid}>
        {/* Clickable Uncovered Lines Panel */}
        <div style={styles.panel}>
          <h4 style={styles.panelTitle}>
            Uncovered Lines ({testRunData.uncoveredLines.length})
          </h4>
          <p style={styles.panelSubtitle}>
            Click an uncovered line to inspect location context.
          </p>

          {testRunData.uncoveredLines.length === 0 ? (
            <div style={styles.successBox}>
              All target code lines and branches are fully covered!
            </div>
          ) : (
            <div style={styles.uncoveredList}>
              {testRunData.uncoveredLines.map((item, idx) => {
                const isSelected =
                  selectedLine?.file === item.file && selectedLine?.line === item.line
                return (
                  <div
                    key={`${item.file}-${item.line}-${idx}`}
                    onClick={() => setSelectedLine(item)}
                    style={{
                      ...styles.uncoveredItem,
                      backgroundColor: isSelected ? '#1e293b' : '#0f172a',
                      borderColor: isSelected ? '#38bdf8' : '#334155',
                    }}
                    data-testid={`uncovered-line-${idx}`}
                  >
          ...
          {selectedLine && (
            <div style={styles.selectedLineDetail} data-testid="selected-line-detail">
              <strong>Selected Location:</strong> {selectedLine.file}:L{selectedLine.line}{' '}
              {selectedLine.branch ? '(Uncovered Branch)' : '(Uncovered Line)'}
            </div>
          )}
        </div>

        {/* Generated Test Code Viewer */}
        <div style={styles.panel}>
          ...
          <pre style={styles.codeBlock} data-testid="test-code-block">
            <code>{testRunData.testCode || '// No test code available.'}</code>
          </pre>
        </div>
      </div>

      {/* Failed Tests Alert Box (if any) */}
      {testRunData.failedTests.length > 0 && (
        <div style={styles.failedSection}>
          <h4 style={{ ...styles.panelTitle, color: '#f87171' }}>
            Failed Test Cases ({testRunData.failedTests.length})
          </h4>
          {testRunData.failedTests.map((failed, idx) => (
            <div key={idx} style={styles.failedCard}>
              <span style={styles.failedName}>{failed.name}</span>
              {failed.targetEntity && (
                <span style={styles.failedEntity}>Target: {failed.targetEntity}</span>
              )}
