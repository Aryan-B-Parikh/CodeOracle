...
def run_pytest() -> float:
    ...
    if result.returncode != 0:
        logger_err = result.stderr or result.stdout
        raise RuntimeError(f"Pytest execution failed with exit code {result.returncode}: {logger_err}")
    if not COVERAGE_JSON.is_file():
        raise RuntimeError(
            "coverage.json was not generated by pytest-cov. "
            "pytest-cov is required for real benchmark coverage measurement."
        )
    ...
    try:
        ...
        raise RuntimeError(f"Failed to parse coverage.json output: {err}")
...
def main() -> int:
    ...
    print("COVERAGE_SOURCE=coverage.py")
    summary_report = {
