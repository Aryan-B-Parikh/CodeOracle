...
## 2026-08-12 — T-12 Impact analysis

- **`app/services/impact.py` + `app/api/routes/entities.py`**: Implemented entity impact analysis service and endpoint `GET /api/v1/repositories/{id}/entities/{entityId}/impact`.
...
## 2026-08-13 — Deep audit fixes (T-02..T-17, "10/10" pass)

Full re-audit of tasks T-01..T-17 (commit `7a5bcf9`) found 22 findings across 17 tasks; the fixes below close every P1/P2. All quality gates re-run after each change.

- **Java sandbox execution (P1, T-02/T-14):** `sandbox_runner` picked `list(repository.languages.keys())[0]` which was always `"python"` (`languages` always contains all three keys), so Java repos silently ran the Python path. Fixed with `_choose_language(repository)`: per-file majority over non-test source files, `python` on tie/mixed — same rule as the test generator. Regression tests added (`test_choose_language_java_repo_not_dict_order`).
- **Fabricated test statistics (P1, T-13/T-15):** the generator faked `TestCase` rows (full-entity-span `coverage_line_nums`, 12/8 ms durations) and `sandbox_runner` computed fake counts (`entities×2`, `-1`). Now: `backend/sandbox/run.py` emits pytest junit.xml / surefire XML while preserving exit codes (`pytest ...; RC=$?; cat ...; exit $RC`) and returns real `tests` stats via `parse_tests_report`; `TestCase` rows are created **only** from that report with real `duration_ms`/`status` and `coverage_line_nums=NULL`; Docker-unavailable runs fail closed (`tests_generated=0`, uncovered `[]`, no invented numbers). `coverage_line_nums` is now nullable (migration `0010_test_case_metrics`).
- **Mock provider honesty (P1, T-09/T-10/T-11):** with no `LLM_API_KEY`, `get_llm_gateway()` silently returns `MockLLMProvider` and explanation/summary answers would look real. `explanation.py`/`summary.py` now branch on real-provider status, return facts-only fallback text (provider null), and **surface `provider` in every payload** (`"mock"`/`"openai"`/`"anthropic"`); consumers can distinguish. All demo-hardcoded tax/`calculate_tax` content removed from explanation; fallback evidence claims cite actual source lines ("Conditional control flow at line N." …).
- **Refactor fabrication (P1, T-17):** `refactor.py` built a fake TODO-docstring "proposal" when the LLM failed. It now raises `RuntimeError` on mock provider / non-JSON output / LLM error; the route maps it to **502** (ValueError → 404 unchanged). Nothing is presented as a proposal unless the LLM returned valid JSON. Tests assert the 502s + `originalChecksum` SHA-256 integrity (original source unchanged).
- **Java test validation (P2, T-13):** generated Java was checked with only a `@Test` substring scan; now uses tree-sitter (`_is_valid_java` via `parse_java`), alongside `_is_valid_python`, in generation and the repair loop.
- **Alembic portability (P2, T-03/T-08):** `0003` emitted PG-only `'{}'::jsonb` and `0008` emitted `ALTER COLUMN ... TYPE` / `CREATE EXTENSION` on any dialect. Now SQLite-safe: server default `sa.text("'{}'")`, dialect-guarded 0008 (PG-only DDL, JSON kept elsewhere), verified via `alembic upgrade head` on SQLite + CI still runs PG16+pgvector.
- **Benchmark honesty (P2):** `benchmark_result.json` never existed (earlier reads were tool corruption); the real benchmark is `benchmark/legacy_demo/python/run_benchmark.py`, now honestly documented as a demo-repo sanity check (the pipeline loop is covered by `test_repair` + the CI Docker job). Baseline assertion band 25–55%; measured baseline 45.7% → 94.3% PASS.
- **Gateway resilience (P2):** `estimate_tokens` no longer lets tiktoken import/network failures crash token budgeting — any exception falls back to the heuristic (kills the flaky `test_openai_provider_success`).
- **Coverage UI unreachable (P2, T-16):** the 4 placeholder tabs (Graph, Impact, Embeddings, Modernization) now render "Not implemented yet (scoped out: …)" instead of dead mock panels; **Tests tab was truly unreachable** (no route/state) — `App.tsx` now wires repository select/upload, tests, and refactor tabs to the real APIs.
- **Repositories list (P2):** added `GET /api/v1/repositories` (newest 50) so the UI can populate the dropdown; `RepositoryListEnvelope`.
- **`repositories` route (P2):** recount on re-upload (`repository.counted = "pending"`) now triggers `scan_repository` instead of the missing `rescan_repository` (was an import-time crash on ZIPs).
- **CI (P2):** backend job gains the coverage benchmark step + `npm run build` in the frontend job; new **sandbox job** builds the Docker image and runs Python/Java `run.py` smoke tests + `test_repair`/`test_sandbox`. Not verifiable locally (no Docker) — CI contract.
